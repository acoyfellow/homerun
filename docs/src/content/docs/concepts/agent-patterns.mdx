...
The normalizer detects four ID patterns:
- **Numeric** — `\d+`
- **UUID** — standard 8-4-4-4-12 hex format
- **Base64** — 16+ chars of `[A-Za-z0-9+/=]`
- **Hex** — 8+ hex characters

Everything else is treated as a literal path segment. This means the agent sees `GET /users/:id` once, not `GET /users/1`, `GET /users/2`, `GET /users/3` separately.
...
When multiple responses are observed for the same endpoint, schemas are **merged**:
- Properties present in all samples → `required`
- Properties present in some samples → optional
- `integer` + `number` → `number`
- Conflicting types → `anyOf`

This means the more traffic the scout captures, the more accurate the schema becomes.
...
A robust agent handles errors like this:

```
worker(pathId, data)
  ├─ success → return result
  ├─ PathBrokenError → heal(pathId) → worker(newPathId, data)
  ├─ NotFoundError → scout(url, task) → worker(newPathId, data)  
  ├─ NetworkError → wait 2s → retry up to 3x
  └─ BrowserError → report failure to user
```
